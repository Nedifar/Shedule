...
namespace wbExample.Controllers
{
    ...
    public class LastDanceController : ControllerBase
    {
        ...
        public ActionResult<IEnumerable<string>> GetNew() //вернуть информацию о новом расписании
        {
            ...
            {
                Differents.DateOut(selectedDate.AddDays(7));
                if (FormatsRaspisanie() == "good")
        ...
        public async Task<ActionResult<DayWeekClass>> GetNewn(string btnContent)
        {
            if (btnContent == "Новое расписание!!!")
            {
                ...
                workbook = new Aspose.Cells.Workbook($"{System.AppDomain.CurrentDomain.BaseDirectory}_{Differents.DdownDay.AddDays(7).Day}_{Differents.DupDay.AddDays(7).Day}.xls");
                workbook.Save($"{System.AppDomain.CurrentDomain.BaseDirectory}_{Differents.DdownDay.AddDays(7).Day}_{Differents.DupDay.AddDays(7).Day}.xlsx", Aspose.Cells.SaveFormat.Xlsx);
                Differents._workbook = new XLWorkbook($"{AppDomain.CurrentDomain.BaseDirectory}Raspisanie/_{Differents.DdownDay.AddDays(7).Day}_{Differents.DupDay.AddDays(7).Day}.xlsx");
                Differents.workSheet = Differents._workbook.Worksheets.First();

            }
            else
            {
                Differents._workbook = new XLWorkbook($"{AppDomain.CurrentDomain.BaseDirectory}Raspisanie/_{Differents.downDay}_{Differents.upDay}.xlsx");
                Differents.workSheet = Differents._workbook.Worksheets.First();
            }
        ...
        {
            var lines = System.IO.File.ReadAllLines($"{AppDomain.CurrentDomain.BaseDirectory}Raspisanie/Formats.txt");
            foreach (string line in lines)
            {
                try
                {
                    string url = "";
                    string[] massiv = Regex.Split(line, @"\s");
