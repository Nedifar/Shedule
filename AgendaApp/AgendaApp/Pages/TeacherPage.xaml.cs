    ...
    public partial class TeacherPage : ContentPage
    {
            ...
            while (5 > 3)
            {
                try
                {
                    if (pCabinet.SelectedIndex != -1)
                    {
                        ...
                        loading.IsAnimationPlaying = true;
                        cvSchedule.IsVisible = false;
                        var resGroup = await http.GetAsync(new Uri($"https://bsite.net/GrishaMasha//api/lastdance/getteacher/{pCabinet.SelectedItem.ToString().Replace("★", "")}"));
        ...
        private async void GetGroupList()
        {
            while (5 > 3)
            {
                try
                {
                    var resGroupList = await http.GetAsync(new Uri($"https://bsite.net/GrishaMasha//api/lastdance/getteachersList"));
